...
def get_opt():
    ...
    parser.add_argument('-b', '--batch-size', type=int, default=1) #8
    parser.add_argument('--fp16', action='store_true', help='use amp')

    parser.add_argument("--dataroot", default="./test")
...
def test(opt, test_loader, board, tocg, D=None):
    ...
    for inputs in test_loader.data_loader:
        ...
        # except KeyError as e:
        #     while(e == 'paired'):
        #         print('waiting for a while to catch the image')
        #         # time.sleep(1)
        #         c_paired = inputs['cloth'][opt.datasetting] #.cuda()
        #         cm_paired = torch.FloatTensor((cm_paired.detach().cpu().numpy() > 0.5).astype(np.float)) #.cuda()

        c_paired = inputs['cloth'][opt.datasetting] #.cuda()
        # while(not inputs['cloth'][opt.datasetting]):
        ...
        for i in range(c_paired.shape[0]):
            ...
            save_image((im_c[i].cpu() / 2 + 0.5), os.path.join('./output','test.png'))
...
def main():
    ...
    print('the test is ----------------->', test_loader)
    # tirar os parametros e passar directo                               
    # visualization
