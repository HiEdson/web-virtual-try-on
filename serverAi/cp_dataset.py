...
class CPDatasetTest(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.fine_width = opt.fine_width
        self.semantic_nc = opt.semantic_nc
        self.data_path = opt.dataroot #osp.join(opt.dataroot, opt.datamode)
        self.transform = transforms.Compose([  \
                transforms.ToTensor(),   \
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    ...
    def __getitem__(self, index):
        im_name = '00891_00.jpg' #self.im_names[index]
        c_name = {}
        c = {}
        cm = {}
        # print('--------------------------------')
        # print(self.c_names['unpaired'])
        # print('--------------------------------')
        for key in self.c_names:
            c_name[key] = self.c_names[key][index]
            # print(self.c_names[key][index])
            try:
                c[key] = Image.open(osp.join(self.data_path, 'cloth', '01260_00.jpg').replace(       #c_name[key]
                    '\\', '/')).convert('RGB')
                c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
                cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask','01260_00.jpg'))
                cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
        ...
        # person image
        try:
            im = Image.open(osp.join(self.data_path, 'image',
                            '00891_00.jpg').replace('\\', '/')) #im_name
            im = transforms.Resize(self.fine_width, interpolation=2)(im)
            im = self.transform(im)
        except Exception as e:
            print('Something went wrong inside person img', e)

        # load parsing image
        parse_name = im_name.replace('.jpg', '.png')
        try:
            im_parse = Image.open(
            osp.join(self.data_path, 'image-parse-v3', parse_name).replace('\\', '/'))
            ...
            print('Something went wrong inside image parse', e)
        ...
        pose_map = Image.open(osp.join(self.data_path, 'openpose-img', pose_name).replace('\\','/'))
        pose_map = transforms.Resize(self.fine_width, interpolation=2)(pose_map)
        pose_map = self.transform(pose_map)  # [-1,1]
        ...
        densepose_map = Image.open(
            osp.join(self.data_path, 'image-densepose', densepose_name).replace('\\', '/'))
        densepose_map = transforms.Resize(self.fine_width, interpolation=2)(densepose_map)
        densepose_map = self.transform(densepose_map)  # [-1,1]
