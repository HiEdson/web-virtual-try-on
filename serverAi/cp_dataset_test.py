...
class CPDatasetTest(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # im_names = ['00891_00.jpg', '03615_00.jpg', '07445_00.jpg']
        ...
        #         temp2 = lineArr[1][0]
        #         print(temp1, temp2)
        #         # im_name, c_name = lineArr[0][0], lineArr[1]
        #         return
        #         # print(lineArr[0])
        #         # im_name = lineArr[0]
        #         # c_name = lineArr[1]
        #         # print(im_name, c_name)
        #         # return
        #         # im_name, c_name = line.split()
        #         # im_names.append(im_name)
        #         # c_names.append(c_name)
        #         im_names.append(temp1)
        #         c_names.append(temp2)
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            #add a temp path
            temp_cloth_path = osp.join(self.data_path, 'cloth', c_name[key]).replace('\\', '/')
            temp_cloth_mask_path = osp.join(self.data_path, 'cloth-mask', c_name[key]).replace('\\', '/')
            # print(temp_cloth_mask_path, temp_cloth_path)
            try:
                c[key] = Image.open(temp_cloth_path).convert('RGB')
                c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
                cm[key] = Image.open(temp_cloth_mask_path)
                cm[key] = transforms.Resize(self.fine_width, interpolation=0)(cm[key])
            except Exception as e:
                print('Something went wrong inside', e)
                continue
        ...
        # person image
        temp_image = osp.join(self.data_path, 'image', im_name).replace('\\', '/')
        im_pil_big = Image.open(temp_image)
        ...
        temp_img_parse = osp.join(
            self.data_path, 'image-parse-v3', parse_name).replace('\\', '/')
        im_parse_pil_big = Image.open(temp_img_parse)
        ...
        # torch.Size([20, 1024, 768]) torch.Size([1, 1024, 768])

        parse_map = torch.FloatTensor(20, self.fine_height, self.fine_width).zero_()
        print('the sape of both are -----> ',parse_map.shape, parse.shape)
        ...
        temp_img_agnostic = osp.join(self.data_path, 'image-parse-agnostic-v3.2', parse_name).replace('\\', '/')
        image_parse_agnostic = Image.open(temp_img_agnostic)
        ...
        temp_densepose = osp.join(self.data_path, 'image-densepose', densepose_name).replace('\\', '/')
        densepose_map = Image.open(temp_densepose)
